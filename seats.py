...
def updateSeatsAvailablePercat(optionList,movieName,category):
    dictValues = {}
    optionList = optionList.split(",")
    rowsList = []
    dictValues = getDictValuesFromFile(movieName)
    #updating
    length_of_optionList = len(optionList)
    if category == "TWIN":
        if length_of_optionList%2 != 0: #adding an extra seat
            lastSeat = optionList[length_of_optionList-1]
            lastSeatLetter = lastSeat[0]
            lastSeatNumber = int(lastSeat[1])
            if lastSeatNumber+1 < 20:
                newLastSeat = lastSeatLetter + str(lastSeatNumber+1)
                optionList.append(newLastSeat)
